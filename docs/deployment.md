# Deployment Guide — Adtech
> Version: 1.0 | Last Updated: 2026-02-18 | Status: Draft
> Classification: Internal | Author: AIDD System

## 1. Overview

Deployment procedures for Adtech across all environments.
...
kubectl label namespace adtech istio-injection=enabled
```

### 3.3 Secrets Configuration
```bash
# Create secrets from Vault
vault kv get -format=json secret/adtech/db | \
...
docker build -t registry.billyrinks.com/adtech:$VERSION .
docker push registry.billyrinks.com/adtech:$VERSION

# Step 2: Deploy to green environment
helm upgrade --install adtech ./charts \
  --namespace adtech \
...
kubectl patch service adtech -p '{"spec":{"selector":{"version":"green"}}}'
```
...
kubectl patch service adtech -p '{"spec":{"selector":{"version":"blue"}}}'

# Or rollback Helm release
helm rollback adtech --namespace adtech
```
...
# Verify metrics
curl http://prometheus:9090/api/v1/query?query=up{job="adtech"}
```
